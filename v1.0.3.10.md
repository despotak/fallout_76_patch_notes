...
> - Alternatively, if the player decides not to place their C.A.M.P. in a new location, and instead joins a new world, their C.A.M.P. will be automatically placed back in its original location-provided that spot is unoccupied.
>
...
- More work done in preparation for the Holidays. We try to spoil as little as possible of this. (Other minor spoilers, for unreleased items, are present)
...
- `Vault-Tec Generator - Small` name conflict with `Generator - Small` fixed
...
- `West Virginia Power Armor Paint Set` description fixed
...
- **Unknown changes to Effect:** `PA Common Arm`, `FieldGen EMP Spell 4`, `Scorchbeast Cloak`, `Nuclear Fallout Radiation`, `Assaultron Rad Damage`
...
[Diff list of the files inside the `.ba2`s](https://raw.githubusercontent.com/despotak/fallout_76_patch_notes/master/file%20diffs/v1.0.3.10_file_diff.txt) (18.322 lines of text)
